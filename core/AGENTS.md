...
3. **Memory 检索**：查询 `taskHistory` 和 `healing.retryHistory`，寻找相似任务和历史修复经验
...
| 时机 | 做什么 |
|------|--------|
| **CP0 INIT** | 必须检索：加载 `taskHistory` 和 `healing.retryHistory` |
| **CP1 PLAN** | 可选：查同类任务的实现方案 |
| **CP2 EXEC 阻塞** | 必须：查 `healing.retryHistory` 中的同类失败记录 |
...
- `/state` - 显示状态
- `/start <任务>` - 开始任务 (CP0 INIT：自动检索相似历史)
- `/verify` - 运行门禁
- `/heal` - 运行 CP4 自愈循环
- `/done` - 完成任务
- `/blocked <原因>` - 报告阻塞
